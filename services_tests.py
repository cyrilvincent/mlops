...
class ModelTest(unittest.TestCase):
    ...
    def test_mnist_predict(self):
        s = sklearn_service.MnistSklearnService("data/mnist/rf-93.pickle")
    ...
    def test_cancer_predict(self):
        s = sklearn_service.CancerSklearnService("data/cancer/rf-94.pickle")
    ...
    def test_mnist_tf(self):
        s = tensorflow_service.MNISTTFService("data/mnist/cnn.h5")
        with open("data/mnist/mnist_0.json") as f:
            matrix = json.load(f)
        res = s.predict(matrix)
        self.assertEqual(0, res)
